    ...
    <>
      ...
      {filteredItems.length > 0 ? (
          ...
          {filteredItems.map(item => (
            <Card key={item.id} className="group overflow-hidden relative">
              <CardContent className="p-0">
                <div className="absolute top-2 right-2 z-10">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => updateClothingItem(item.id, { pinned: !item.pinned })}>
                ...
                {item.pinned && (
                  <div className="absolute top-2 left-2 z-10 bg-primary rounded-full p-1.5">
                ...
                <div className="p-4 border-t">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{item.color} · {item.occasion}</p>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <Card className="col-span-full flex flex-col items-center justify-center p-12 text-center">
          ...
          <p className="mt-2 text-muted-foreground">Your closet is empty or no items match your search.</p>
