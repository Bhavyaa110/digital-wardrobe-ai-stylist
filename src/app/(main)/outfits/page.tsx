            ...
            <Card key={outfit.id} className="overflow-hidden group relative">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="secondary"
                      size="icon"
                      className="h-8 w-8 bg-black/20 hover:bg-black/50 border-none text-white"
                    >
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() =>
                        updateOutfit({ ...outfit, pinned: !outfit.pinned })
                      }
                    >
                      {outfit.pinned ? (
              ...
              <CardContent className="p-0">
                <div className="grid grid-cols-2 grid-rows-2 aspect-square bg-secondary">
                  {outfit.items
                    .slice(0, 4)
                    .map((item: import("../../../lib/types").ClothingItem) => (
                      <div
                        key={item.id}
                  ...
                  {outfit.items.length < 4 &&
                    [...Array(4 - outfit.items.length)].map((_, i) => (
                      <div
                        key={i}
                        className="bg-secondary border-2 border-background"
                      ></div>
                    ))}
                </div>
